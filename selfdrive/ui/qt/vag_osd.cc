...
VagOsd::VagOsd(QWidget *parent) : QWidget(parent) {
  sm = std::make_unique<SubMaster, const std::initializer_list<const char *>>({"carState", "vagControl"});
}
...
void VagOsd::drawOsdBrakeLight(QPainter &p) {
  ...
  const bool brakeLights = (*s->sm)["carState"].getCarState().getVagCarState().getBrakeLights();
...
void VagOsd::drawOsdBlindspot(QPainter &p) {
  ...
  const bool leftBlindspotWarning = (*s->sm)["carState"].getCarState().getVagCarState().getLeftBlindspotWarning();
  const bool rightBlindspotWarning = (*s->sm)["carState"].getCarState().getVagCarState().getRightBlindspotWarning();
...
void VagOsd::drawOsdNoLeadCar(QPainter &p) {
  ...
  //----- Radar -----
  const bool accEnable = (bool)(*s->sm)["carState"].getCarState().getCruiseState().getEnabled();
  const int accAbstandsindex = (int)(*s->sm)["carState"].getCarState().getVagCarState().getVagUiField().getAccAbstandsindex();
...
void VagOsd::drawOsdLeadCarGoing(QPainter &p) {
  ...
  const bool leadCarGoingTrigged = (bool) (*s->sm)["vagControl"].getVagControl().getLeadCarGoingTrigged();
  ...
  if(leadCarGoingTrigged || isVagDebugLeadCarGoingRemindTest) {
...
void VagOsd::drawOsdTest(QPainter &p) {
  ...
  const int accAbstandsindex = (*s->sm)["carState"].getCarState().getVagCarState().getVagUiField().getAccAbstandsindex();
  ...
  const float obdEngCoolTemp = (*s->sm)["carState"].getCarState().getVagCarState().getVagUiField().getObdEngCoolTemp();
